...
parser.add_argument('--n_jobs', type=int, help='the number of jobs to run in parallel. Default=1', default=1)
...
searcher = GridSearchCV(pipeline, parameters, scoring=make_scorer(f1_score), n_jobs=args.n_jobs, verbose=1, cv=KFold(X.shape[0])) #default StratifiedKFold doesn't work with multiclass classification
